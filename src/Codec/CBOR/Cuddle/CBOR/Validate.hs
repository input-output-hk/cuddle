...
doValidate cddl t theRule = case resolveIfRef cddl theRule of
  ...
  _ -> case t of
    TInt i -> case resolveIfRef cddl theRule of
      ...
      Control op n ctrller ->
        let intControl = case (op, resolveIfRef cddl ctrller) of
              (Lt, Literal (VUInt i')) -> if i < fromIntegral i'
                                           then _Success # ()
                                           else _Failure # [DidNotValidate "Failed control"]
              (Lt, Literal (VNInt i')) -> if i < - (fromIntegral i')
                                           then _Success # ()
                                           else _Failure # [DidNotValidate "Failed control"]
              (Le, Literal (VUInt i')) -> if i <= fromIntegral i'
                                           then _Success # ()
                                           else _Failure # [DidNotValidate "Failed control"]
              (Le, Literal (VNInt i')) -> if i <= - (fromIntegral i')
                                           then _Success # ()
                                           else _Failure # [DidNotValidate "Failed control"]
              (Gt, Literal (VUInt i')) -> if i > fromIntegral i'
                                           then _Success # ()
                                           else _Failure # [DidNotValidate "Failed control"]
              (Gt, Literal (VNInt i')) -> if i > - (fromIntegral i')
                                           then _Success # ()
                                           else _Failure # [DidNotValidate "Failed control"]
              (Ge, Literal (VUInt i')) -> if i >= fromIntegral i'
                                           then _Success # ()
                                           else _Failure # [DidNotValidate "Failed control"]
              (Ge, Literal (VNInt i')) -> if i >= - (fromIntegral i')
                                           then _Success # ()
                                           else _Failure # [DidNotValidate "Failed control"]
              (Eq, Literal (VUInt i')) -> if i == fromIntegral i'
                                           then _Success # ()
                                           else _Failure # [DidNotValidate "Failed control"]
              (Eq, Literal (VNInt i')) -> if i == - (fromIntegral i')
                                           then _Success # ()
                                           else _Failure # [DidNotValidate "Failed control"]
              (Ne, Literal (VUInt i')) -> if i /= fromIntegral i'
                                           then _Success # ()
                                           else _Failure # [DidNotValidate "Failed control"]
              (Ne, Literal (VNInt i')) -> if i /= - (fromIntegral i')
                                           then _Success # ()
                                           else _Failure # [DidNotValidate "Failed control"]
              _ -> error "nonsense"
    ...
    TInteger i -> case resolveIfRef cddl theRule of
      ...
      Control op n ctrller ->
        let intControl = case (op, resolveIfRef cddl ctrller) of
              (Lt, Literal (VBignum i')) -> if i < i'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              (Le, Literal (VBignum i')) -> if i <= i'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              (Gt, Literal (VBignum i')) -> if i > i'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              (Ge, Literal (VBignum i')) -> if i >= i'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              (Eq, Literal (VBignum i')) -> if i == i'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              (Ne, Literal (VBignum i')) -> if i /= i'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              _ -> error "nonsense"
        in case resolveIfRef cddl n of
    ...
    TBytes b -> case resolveIfRef cddl theRule of
      ...
      Control op n ctrller ->
        let bytesControl = case (op, resolveIfRef cddl ctrller) of
              (Size, Literal (VUInt i)) -> if BS.length b == fromIntegral i
                                           then _Success # ()
                                           else _Failure # [DidNotValidate "Failed control"]
              (Lt, Literal (VBytes b')) -> if b < b'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              (Le, Literal (VBytes b')) -> if b <= b'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              (Gt, Literal (VBytes b')) -> if b > b'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              (Ge, Literal (VBytes b')) -> if b >= b'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              (Eq, Literal (VBytes b')) -> if b == b'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              (Ne, Literal (VBytes b')) -> if b /= b'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              _ -> error "nonsense"
        in case resolveIfRef cddl n of
    ...
    TString s -> case resolveIfRef cddl theRule of
      ...
      Control op n ctrller ->
        let textControl = case (op, resolveIfRef cddl ctrller) of
              (Size, Literal (VUInt i)) -> if T.length s == fromIntegral i
                                           then _Success # ()
                                           else _Failure # [DidNotValidate "Failed control"]
              (Lt, Literal (VText b')) -> if s < b'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              (Le, Literal (VText b')) -> if s <= b'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              (Gt, Literal (VText b')) -> if s > b'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              (Ge, Literal (VText b')) -> if s >= b'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              (Eq, Literal (VText b')) -> if s == b'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              (Ne, Literal (VText b')) -> if s /= b'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              _ -> error "nonsense"
        in case resolveIfRef cddl n of
    ...
    TBool b -> case resolveIfRef cddl theRule of
      ...
      Control op n ctrller ->
        let boolControl = case (op, resolveIfRef cddl ctrller) of
              (Eq, Literal (VBool b')) -> if b == b'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              (Ne, Literal (VBool b')) -> if b /= b'
                                             then _Success # ()
                                             else _Failure # [DidNotValidate "Failed control"]
              _ -> error "nonsense"
        in case resolveIfRef cddl n of
